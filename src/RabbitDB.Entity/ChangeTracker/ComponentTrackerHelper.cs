...
namespace RabbitDB.Entity.ChangeTracker
{
    ...
    internal class ComponentTrackerHelper
    {
        ...
        public object AccessProperty(object target, string propertyName)
        {
            ...
            throw new Exception($"Could not access property {propertyName} on {target.GetType() .FullName}");
        }
        ...
        {
            string fullPropertyName = target.GetType()
            ...
            accessDelegate = Expression.Lambda<Func<object, object>>(returnCastExpression, inParameter)
                                       .Compile();
