...
namespace RabbitDB.Expressions
{
    ...
    internal class ExpressionWriter<T> : ExpressionVisitor
    {
        ...
        internal object[] Parameters
        {
            get
            {
                return _parameterCollection.ToArray();
            }
        }
            ...
            else if (q == null)
            {
                switch (Type.GetTypeCode(node.Value.GetType()))
                {
                    case TypeCode.Boolean:
                        _sqlBuilder.Append(((bool)node.Value) ? 1 : 0);
                        break;
                    ...
                    case TypeCode.Object:
                        throw new NotSupportedException(
                            string.Format("The constant for '{0}' is not supported", node.Value));
                    default:
            ...
            switch (node.NodeType)
            {
                case ExpressionType.Not:
                        ...
                        && node.Operand.NodeType == ExpressionType.MemberAccess)
                    {
                        ...
                        if (oper != null && oper.Type != typeof(bool))
                        {
                            break;
                        }
                ...
                default:
                    throw new NotSupportedException(
                        string.Format("The unary operator '{0}' is not supported", node.NodeType));
            }
