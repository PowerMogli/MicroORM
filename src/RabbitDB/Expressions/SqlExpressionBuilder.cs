...
namespace RabbitDB.Expressions
{
    ...
    internal class SqlExpressionBuilder<T>
    {
        ...
        ///     The sql dialect.
        /// </param>
        internal SqlExpressionBuilder(ISqlDialect sqlDialect)
        {
            _sqlDialect = sqlDialect;

            _tableInfo = TableInfo<T>.GetTableInfo;

            _expressionWriter = new ExpressionWriter<T>(sqlDialect.BuilderHelper, Parameters);
        }
        ...
        ///     Gets the parameters.
        /// </summary>
        internal ExpressionParameterCollection Parameters => _parameterCollection ?? (_parameterCollection = new ExpressionParameterCollection());
        ...
        {
            _sqlQuery.Append(_order
                ? ", "
                : " ORDER BY ");

            string column = selector.Body.GetPropertyName();

            _sqlQuery.Append($"{_sqlDialect.SqlCharacters.EscapeName(column)} {SortToString(sort)}");
        ...
        {
            string name = selector.Body.GetPropertyName();
            if (_hasColumn)
            {
                _sqlQuery.Append(", ");
            }
        ...
        {
            SelectSqlBuilder sqlBuilder = new SelectSqlBuilder(_sqlDialect, _tableInfo);
